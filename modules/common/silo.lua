...
function Silo.on_player_joined_game(event)
    local player = game.players[event.player_index]
    Silo.draw_rocket_score_btn(player)
    Silo.draw_rocket_score_frame(player)
end
...
function Silo.on_player_left_game(event)
    local player = game.players[event.player_index]
...
function Silo.on_gui_click(event)
    local player = game.players[event.player_index]
...
--- @param event defines.events.on_rocket_launched
function Silo.on_rocket_launched(event)
    game.print({'silo.rocket_launched'})

    for i, player in pairs(game.connected_players) do
        Silo.update_score(player)
    end
end

-- Event Registration --
-- ======================================================= --
Event.register(defines.events.on_player_joined_game, Silo.on_player_joined_game)
Event.register(defines.events.on_player_left_game, Silo.on_player_left_game)
Event.register(defines.events.on_gui_click, Silo.on_gui_click)
Event.register(defines.events.on_rocket_launched, Silo.on_rocket_launched)
...
function Silo.draw_rocket_score_btn(player)
    if mod_gui.get_button_flow(player)['btn_menu_score'] == nil then
...
--- Draws a small frame next to rocket button to show rocket count
--- @param player LuaPlayer
function Silo.draw_rocket_score_frame(player)
...
--- @param player LuaPlayer
function Silo.update_score(player)
    local frame = mod_gui.get_button_flow(player)['frame_menu_score']
